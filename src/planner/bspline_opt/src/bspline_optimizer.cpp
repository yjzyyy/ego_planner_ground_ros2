...
namespace ego_planner
{
    ...
    std::vector<std::vector<Eigen::Vector3d>> BsplineOptimizer::initControlPoints(Eigen::MatrixXd &init_points, bool flag_first_init /*= true*/)
    {
        ...
        std::cout<<"grid_map_->getResolution(): "<<grid_map_->getResolution()<<std::endl;
        std::cout<<"init_points.col(0) and init_points.rightCols(1):"<<init_points.col(0)<<","<<init_points.rightCols(1)<<std::endl;
    ...
    bool BsplineOptimizer::check_collision_and_rebound(void)
    {
        ...
        for (int i = order_ - 1; i <= i_end; ++i)
        {
            ...
            if (occ)
            {
                ...
                {
                    RCLCPP_WARN(rclcpp::get_logger("check_collision_and_rebound"),"WARN! terminal point of the current trajectory is in obstacle, skip this planning.");
    ...
    bool BsplineOptimizer::rebound_optimize()
    {
        ...
        do
        {
            ...
            {
                RCLCPP_WARN(rclcpp::get_logger("rebound_optimize"),"Solver error. Return = %d, %s. Skip this planning.", result, lbfgs::lbfgs_strerror(result));
                // while (ros::ok());
            }
    ...
    bool BsplineOptimizer::refine_optimize()
    {
        ...
        do
        {
            ...
            {
                RCLCPP_ERROR(rclcpp::get_logger("refine_optimize"),"Solver error in refining!, return = %d, %s", result, lbfgs::lbfgs_strerror(result));
            }
