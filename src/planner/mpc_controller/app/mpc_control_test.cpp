...
rclcpp::Publisher<nav_msgs::msg::Path>::SharedPtr trajectory_predict_pub;
rclcpp::Publisher<geometry_msgs::msg::PointStamped>::SharedPtr start_point_pub;
rclcpp::Publisher<geometry_msgs::msg::PointStamped>::SharedPtr predict_end_point_pub;

// 消息对象定义（ROS 2消息命名空间有变化）
nav_msgs::msg::Path b_spline_trajectory;
nav_msgs::msg::Path trajectory_predict;
geometry_msgs::msg::PointStamped start_point;
geometry_msgs::msg::PointStamped predict_end_point;
...
int main(int argc, char** argv) {
    ...
    node_ = rclcpp::Node::make_shared("mpc_test_node","mpc_test");
...
//    start_point.point.y = 0.472;
    ...
    trajectory_pub = node_->create_publisher<nav_msgs::msg::Path>("/Bspline_trajectory", 10);
    trajectory_predict_pub = node_->create_publisher<nav_msgs::msg::Path>("/trajectory_predict", 10);
    start_point_pub = node_->create_publisher<geometry_msgs::msg::PointStamped>("/start_point", 10);
    predict_end_point_pub = node_->create_publisher<geometry_msgs::msg::PointStamped>("/predict_point", 10);
...
//    X_k(1) = 0.472-y_offset;
    ...
    rclcpp::Time t_end = rclcpp::Clock().now();
    RCLCPP_INFO(node_->get_logger(),"control total time : %5.3f ms",(t_end-t_start).seconds()*1000);
