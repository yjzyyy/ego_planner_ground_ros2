...
{
    // 从节点获取参数（ROS 2参数获取方式）
    // 第一个参数：参数名（ROS 2中参数通常使用相对路径，相对于节点命名空间）
    // 第二个参数：存储参数值的变量
    // 第三个参数：默认值（可选，当参数未设置时使用）
    node->get_parameter_or("MPC/v_max", v_max, 1.8);
    node->get_parameter_or("MPC/w_max", w_max, 1.2);
    node->get_parameter_or("MPC/omega0", omega0, 1.0);
    node->get_parameter_or("MPC/omega1", omega1, 0.1);

    // 推荐写法：带默认值的参数获取（避免参数未设置时出错）
    // node->get_parameter_or("/o_egplanner_node/MPC/v_max", v_max, 0.7);
    // node->get_parameter_or("/ego_planner_node/MPC/w_max", w_max, 0.3);
