...
namespace ego_planner
{
  ...
  void EGOReplanFSM::init(rclcpp::Node::SharedPtr &node)
  {
    ...
    node->declare_parameter("fsm/planning_horizen_time", -1.0);  // 注意：horizen可能是horizon的拼写错误
    node->declare_parameter("fsm/emergency_time_", 1.0);
    node->declare_parameter("fsm/w_adjust_", 1.0);
    ...
    node->get_parameter("fsm/planning_horizon", planning_horizen_);
    node->get_parameter("fsm/planning_horizen_time", planning_horizen_time_);
    node->get_parameter("fsm/emergency_time_", emergency_time_);
    node->get_parameter("fsm/w_adjust_", w_adjust);
    ...
    cmd_pub_ = node_->create_publisher<geometry_msgs::msg::TwistStamped>("cmd_vel", 100);
    adjust_cmd_pub_ = node_->create_publisher<std_msgs::msg::UInt8>("is_adjust_yaw", 100);
    odom_adjust_pub_ = node_->create_publisher<nav_msgs::msg::Odometry>("odom_adjust", 100);
    dir_pub = node_->create_publisher<std_msgs::msg::UInt8>("direction", 100);
    stop_pub = node_->create_publisher<std_msgs::msg::UInt8>("emergency_stop", 100);
  ...
  void EGOReplanFSM::planGlobalTrajbyGivenWps()
  {
    ...
    {
      RCLCPP_ERROR(node_->get_logger(),"cancle to generate global trajectory!");
    }
  }

  void EGOReplanFSM::waypointCallback(const nav_msgs::msg::Path::SharedPtr &msg)
    {
...
void EGOReplanFSM::goal_callback(const std::shared_ptr<const geometry_msgs::msg::PoseStamped> &msg)
{
...
//    }
  ...
  void EGOReplanFSM::odometryCallback(const std::shared_ptr<const nav_msgs::msg::Odometry> &msg)
  {
    ...
    if(dir==NEGATIVE)
    {
      ...
      // 创建四元数（roll, pitch, yaw为输入的欧拉角）
      tf_quat.setRPY(roll, pitch, yaw);
      // 转换为ROS 2消息类型的四元数
      tf_quat.normalize();
      geometry_msgs::msg::Quaternion quat = tf2::toMsg(tf_quat);

      // 复制原始消息内容（msg应为nav_msgs::msg::Odometry::SharedPtr类型）
      odom_adjust = *msg;
      // 更新朝向信息
...
//        }
    ...
    {
      start_pt_ = odom_pos_;
      std::cout<<"start pt : "<<start_pt_ <<std::endl;
      start_vel_ << 0,0,0;
      start_acc_.setZero();
      ...
      std::cout<<"flag_random_poly_init : "<<flag_random_poly_init <<std::endl;
  ...
  void EGOReplanFSM::getLocalTarget()
  {
    ...
    for (t = planner_manager_->global_data_.last_progress_time_; t < planner_manager_->global_data_.global_duration_; t += t_step)
    {
      ...
      {
        // todo
        RCLCPP_ERROR(node_->get_logger(),"last_progress_time_ ERROR !!!!!!!!!");
        RCLCPP_ERROR(node_->get_logger(),"last_progress_time_ ERROR !!!!!!!!!");
        return;
      }
      if (dist < dist_min)
